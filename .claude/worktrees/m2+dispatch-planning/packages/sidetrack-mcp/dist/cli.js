...
import { sidetrackMcpHttpPort, startStreamableHttpMcpServer, } from './server/streamableHttpServer.js';
...
const createCompanionWriteClient = (companionUrl, bridgeKey) => {
    ...
    return {
        ...
        async requestDispatch(input) {
            ...
            const body = await post('/v1/dispatches', {
                kind: 'coding',
                ...
                'idempotency-key': idempotencyKey('mcp-dispatch', [
        ...
        async moveThread(input) {
            ...
            const body = await post('/v1/threads', upsert, { 'x-sidetrack-mcp-tool': 'sidetrack.threads.move' });
        ...
        async createAnnotation(input) {
            ...
            const sourceTurnKey = input.sourceTurn === undefined
                ? ''
            ...
            const policyKey = policy === undefined
                ? ''
                : `${policy.repeatedTerm ?? ''}:${String(policy.shortTermMinLength ?? '')}`;
            const body = await post('/v1/annotations', requestBody, {
                'x-sidetrack-mcp-tool': 'sidetrack.annotations.create',
                'idempotency-key': idempotencyKey('mcp-annotation', [
            ...
            if (status === 'created') {
                ...
                return {
                    ...
                    annotation: typeof annotation === 'object' && annotation !== null
                        ? annotation
                        : {},
            ...
            if (status === 'anchor_failed' || status === 'validation_failed') {
                ...
                if (reasonValue === undefined) {
                    throw new Error(`Companion create_annotation returned unrecognised reason: ${String(reason)}`);
                }
                ...
                return {
                    ...
                    occurrenceCount: typeof data['occurrenceCount'] === 'number'
                        ? data['occurrenceCount']
                        : 0,
                    ...(Array.isArray(suggestedRaw) &&
                        suggestedRaw.every((entry) => typeof entry === 'string')
                        ? { suggestedSelectionHints: suggestedRaw }
                        : {}),
                };
            }
            throw new Error(`Companion create_annotation returned unrecognised status: ${String(status)}`);
        },
        ...
        async awaitCaptureForDispatch(input) {
            const url = new URL(`${base}/v1/dispatches/${encodeURIComponent(input.dispatchId)}/await-capture`);
