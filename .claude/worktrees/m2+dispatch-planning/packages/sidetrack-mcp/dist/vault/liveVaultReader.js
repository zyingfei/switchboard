...
const ensureInsideRoot = (rootPath, childPath) => {
...
const readJsonDirectory = async (rootPath, directory, schema) => {
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
        ...
        values.push(schema.parse(JSON.parse(raw)));
    }
    return values;
};
const readEventLogs = async (rootPath) => {
...
export class LiveVaultReader {
    ...
    async readSnapshot() {
        ...
        return {
            threads: threads.sort((left, right) => (right.lastSeenAt ?? '').localeCompare(left.lastSeenAt ?? '')),
            workstreams,
            queueItems,
            reminders,
            events,
    ...
    async readDispatches(options = {}) {
        ...
        const events = (await Promise.all(entries
            ...
            .slice(0, 100)
            .map((entry) => readDispatchFile(join(dispatchDirectory, entry.name))))).flat();
        return {
            data: events
    ...
    async readCodingSessions(options = {}) {
        const sessions = await readJsonDirectory(this.vaultPath, '_BAC/coding/sessions', codingSessionSchema);
        return sessions
            .filter((session) => (options.workstreamId === undefined || session.workstreamId === options.workstreamId) &&
    ...
    async readReviews(options = {}) {
        ...
        const events = (await Promise.all(entries
            ...
            .sort((left, right) => right.name.localeCompare(left.name))
            .slice(0, 100)
            .map((entry) => readReviewFile(join(reviewDirectory, entry.name))))).flat();
        return {
            data: events
                .filter((event) => (sinceMillis === undefined || Date.parse(event.createdAt) >= sinceMillis) &&
...
//# sourceMappingURL=liveVaultReader.js.map