...
export const createSidetrackMcpServer = (reader, companionClient) => {
    ...
    server.registerTool('sidetrack.threads.list', {
    ...
    server.registerTool('sidetrack.workstreams.get', {
    ...
    server.registerTool('sidetrack.workstreams.context_pack', {
    ...
    server.registerTool('sidetrack.search', {
    ...
    server.registerTool('sidetrack.queue.list', {
    ...
    server.registerTool('sidetrack.reminders.list', {
    ...
    server.registerTool('sidetrack.sessions.list', {
    ...
    server.registerTool('sidetrack.threads.move', {
        ...
        if (companionClient?.moveThread === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.threads.move is unavailable.');
        }
    ...
    server.registerTool('sidetrack.queue.create', {
        ...
        if (companionClient?.createQueueItem === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.queue.create is unavailable.');
        }
        if (scope !== 'global' && (targetId === undefined || targetId.length === 0)) {
            throw new Error(`sidetrack.queue.create requires targetId when scope='${scope}'. Pass the thread or workstream bac_id, or use scope='global'.`);
        }
    ...
    server.registerTool('sidetrack.workstreams.bump', {
        ...
        if (companionClient?.bumpWorkstream === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.workstreams.bump is unavailable.');
        }
        return asStructuredContent(await companionClient.bumpWorkstream({ bac_id }));
    });
    server.registerTool('sidetrack.threads.archive', {
        description: 'Soft-archive a tracked thread. Idempotent.',
        inputSchema: { bac_id: z.string().min(1) },
    }, async ({ bac_id }) => {
        if (companionClient?.archiveThread === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.threads.archive is unavailable.');
        }
        return asStructuredContent(await companionClient.archiveThread({ bac_id }));
    });
    server.registerTool('sidetrack.threads.unarchive', {
        description: 'Clear a thread soft-archive marker. Idempotent.',
        inputSchema: { bac_id: z.string().min(1) },
    }, async ({ bac_id }) => {
        if (companionClient?.unarchiveThread === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.threads.unarchive is unavailable.');
        }
    ...
    server.registerTool('sidetrack.audit.list', {
        ...
        if (companionClient?.listAuditEvents === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.audit.list is unavailable.');
        }
    ...
    server.registerTool('sidetrack.workstreams.notes', {
        ...
        if (companionClient?.listWorkstreamNotes === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.workstreams.notes is unavailable.');
        }
    ...
    server.registerTool('sidetrack.annotations.list', {
        ...
        if (companionClient?.listAnnotations === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.annotations.list is unavailable.');
        }
    ...
    server.registerTool('sidetrack.annotations.update', {
        ...
        if (companionClient?.updateAnnotation === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.annotations.update is unavailable.');
        }
        return asStructuredContent(await companionClient.updateAnnotation({ bac_id, note }));
    });
    server.registerTool('sidetrack.annotations.delete', {
        description: 'Soft-delete an annotation. Idempotent; history is preserved.',
        inputSchema: { bac_id: z.string().min(1) },
    }, async ({ bac_id }) => {
        if (companionClient?.deleteAnnotation === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.annotations.delete is unavailable.');
        }
    ...
    server.registerTool('sidetrack.recall.query', {
        ...
        if (companionClient?.recall === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.recall.query is unavailable.');
        }
    ...
    server.registerTool('sidetrack.suggestions.workstream', {
        ...
        if (companionClient?.suggestWorkstream === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.suggestions.workstream is unavailable.');
        }
    ...
    server.registerTool('sidetrack.settings.export', {
        description: 'Export portable Sidetrack settings and workstream metadata.',
        inputSchema: {},
    }, async () => {
        if (companionClient?.exportSettings === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.settings.export is unavailable.');
        }
        return asStructuredContent(await companionClient.exportSettings());
    });
    server.registerTool('sidetrack.system.update_check', {
        description: 'Return read-only companion version update advisory.',
        inputSchema: {},
    }, async () => {
        if (companionClient?.systemUpdateCheck === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.system.update_check is unavailable.');
        }
        return asStructuredContent(await companionClient.systemUpdateCheck());
    });
    server.registerTool('sidetrack.buckets.list', {
        description: 'List companion multi-vault routing buckets. Read-only.',
        inputSchema: {},
    }, async () => {
        if (companionClient?.listBuckets === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.buckets.list is unavailable.');
        }
        return asStructuredContent({ items: [...(await companionClient.listBuckets())] });
    });
    server.registerTool('sidetrack.system.health', {
        description: 'Return best-effort companion health metrics for diagnostics.',
        inputSchema: {},
    }, async () => {
        if (companionClient?.systemHealth === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.system.health is unavailable.');
        }
    ...
    server.registerTool('sidetrack.dispatches.list', {
    ...
    server.registerTool('sidetrack.reviews.list', {
    ...
    server.registerTool('sidetrack.threads.turns', {
