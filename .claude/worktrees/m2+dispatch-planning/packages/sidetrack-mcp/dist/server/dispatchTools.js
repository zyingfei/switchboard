...
const buildFormattingPrefix = (captureProfile, responseProfile) => {
    const lines = [];
    ...
    if (responseProfile?.preferSectionHeadings === true &&
...
export const registerDispatchTools = (server, reader, companionClient) => {
    server.registerTool('sidetrack.dispatch.create', {
        ...
        inputSchema: {
            ...
            body: z
                ...
                .min(1)
                .max(20000)
                .describe("Natural-language request to send to the target provider. Should read like a normal user message; let captureProfile / responseProfile carry formatting constraints instead of inline instructions."),
            captureProfile: captureProfileSchema
                .optional()
                .describe("Preset that shapes the response for downstream Sidetrack use. 'annotation_friendly' asks for plain text + ASCII diagrams + section headings + stable terminology — recommended whenever the captured answer will be annotated."),
            responseProfile: responseProfileSchema
                .optional()
                .describe('Fine-grained alternative to captureProfile. Each flag adds a single formatting request. Use this only when no preset fits.'),
            workstreamId: z
                .string()
                .optional()
    ...
    }, async ({ codingSessionId, targetProvider, title, body, captureProfile, responseProfile, workstreamId, sourceThreadId, mode, }) => {
        if (companionClient?.requestDispatch === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.dispatch.create is unavailable.');
        }
        ...
        if (session === undefined) {
            throw new Error(`sidetrack.dispatch.create requires an attached coding session; '${codingSessionId}' is not attached.`);
        }
    ...
    server.registerTool('sidetrack.dispatch.await_capture', {
        description: "Wait for a dispatch to be linked to a captured thread. Use this immediately after sidetrack.dispatch.create when the user expects the target AI's response to be read or annotated. Returns thread identity, resource URIs the agent can readResource, and the latest assistant turn so no follow-up turn fetch is needed.",
        inputSchema: {
            dispatchId: z
                .string()
                .min(1)
            ...
            includeLatestAssistantTurn: z
                .boolean()
                .optional()
                .describe('When true (default), the response includes latestAssistantTurn so the agent can act on the captured answer immediately. Set false to skip the read.'),
        },
        ...
        if (companionClient?.awaitCaptureForDispatch === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.dispatch.await_capture is unavailable.');
        }
        ...
        const resourceLinks = result.matched && result.resources !== undefined && result.thread !== undefined
            ? [
                {
                    type: 'resource_link',
...
//# sourceMappingURL=dispatchTools.js.map