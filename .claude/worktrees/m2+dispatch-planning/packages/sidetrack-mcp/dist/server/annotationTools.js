...
const annotationItemShape = z.object({
    term: z
        ...
        .min(1)
        .max(400)
        .describe('Exact visible term from the captured assistant turn. Prefer precise multi-word phrases over generic single words.'),
    note: z.string().max(5000).describe('Annotation note to display in Sidetrack.'),
    selectionHint: z
        .string()
        .max(512)
        .optional()
        .describe("Optional disambiguator. Use 'ordinal:N' (1-based) for the Nth occurrence, or a short preceding-text fragment. Required only when the tool reports short/repeated-term failure."),
});
...
export const registerAnnotationTools = (server, companionClient) => {
    server.registerTool('sidetrack.annotations.create_batch', {
        description: 'Create 1..20 annotations on a captured Sidetrack thread from semantic intent (exact visible term + note). The companion looks up the thread, picks the source turn, builds the anchor, and writes the annotation. The model must not compute offsets, prefix/suffix, or CSS selectors. Per-item statuses include retry-able anchor_failed reasons with suggestedSelectionHints — retry once with one of those hints when the reason is short_term_requires_selection_hint or ambiguous_term_requires_selection_hint.',
        inputSchema: {
            threadId: z
                .string()
                .min(1)
                .optional()
                .describe('Captured Sidetrack thread bac_id (returned by sidetrack.dispatch.await_capture as thread.threadId). Preferred over url. When provided, the companion resolves url/pageTitle from the thread record.'),
            url: z
                .url()
                .optional()
                .describe('Page URL where annotations restore. Optional when threadId is supplied. Required when threadId is omitted (legacy URL-driven path).'),
            pageTitle: z
                .string()
                .min(1)
                .optional()
            ...
            sourceTurn: sourceTurnShape
                .optional()
                .describe("Which captured turn is the anchor source. Defaults to 'assistant_latest'. Pass 'assistant_all' to span every assistant turn or {ordinal:N} for a specific one."),
            items: z
        ...
        if (companionClient?.createAnnotation === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; sidetrack.annotations.create_batch is unavailable.');
        }
        ...
        const failedCount = results.filter((entry) => entry.status === 'failed' || entry.status === 'validation_failed').length;
        const structured = {
            ...(threadId === undefined ? {} : { threadId }),
            ...(url === undefined ? {} : { url }),
...
//# sourceMappingURL=annotationTools.js.map