...
export const registerPrompts = (server) => {
    ...
    server.registerPrompt('sidetrack.session.attach', {
        title: 'Attach a coding session to Sidetrack',
        description: 'Emits the minimal attach instruction. The agent calls sidetrack.session.attach with the supplied token and continues with the user task.',
        argsSchema: {
    ...
    server.registerPrompt('sidetrack.demo.dispatch_and_annotate', {
        title: 'Dispatch a packet and pin architect-relevant annotations',
        description: 'Sends a dispatch to the chosen provider, awaits the resulting captured thread, then annotates 3-5 architect-relevant terms on the thread URL.',
        argsSchema: {
            targetProvider: z
    ...
    server.registerPrompt('sidetrack.thread.annotate', {
        title: 'Annotate architect-relevant terms on an existing thread',
        description: 'Reads the thread, picks 3-5 architect-relevant terms, and pins them via sidetrack.annotations.create_batch. No dispatch step.',
        argsSchema: {
