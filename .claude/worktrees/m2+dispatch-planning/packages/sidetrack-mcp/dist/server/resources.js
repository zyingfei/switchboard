...
const markdownContents = (uri, markdown) => ({
    contents: [
        {
            uri: uri.toString(),
...
const requireVariable = (variables, key) => {
...
export const registerResources = (server, reader, companionClient) => {
    // sidetrack://thread/{threadId}
    // Thread metadata (title, threadUrl, provider, lastSeenAt, …)
    // pulled from the live vault snapshot.
    const threadRead = async (uri, variables) => {
        const threadId = requireVariable(variables, 'threadId');
    ...
    server.registerResource('sidetrack-thread', new ResourceTemplate('sidetrack://thread/{threadId}', { list: undefined }), {
    ...
    const turnsRead = async (uri, variables) => {
        const threadId = requireVariable(variables, 'threadId');
        const snapshot = await reader.readSnapshot();
        const thread = snapshot.threads.find((entry) => entry.bac_id === threadId);
        if (thread === undefined) {
    ...
    server.registerResource('sidetrack-thread-turns', new ResourceTemplate('sidetrack://thread/{threadId}/turns', { list: undefined }), {
    ...
    const annotationsRead = async (uri, variables) => {
        const threadId = requireVariable(variables, 'threadId');
        if (companionClient?.listAnnotations === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; thread annotations resource is unavailable.');
        }
        const snapshot = await reader.readSnapshot();
        const thread = snapshot.threads.find((entry) => entry.bac_id === threadId);
        if (thread === undefined) {
    ...
    server.registerResource('sidetrack-thread-annotations', new ResourceTemplate('sidetrack://thread/{threadId}/annotations', { list: undefined }), {
    ...
    const threadMarkdownRead = async (uri, variables) => {
        const threadId = requireVariable(variables, 'threadId');
        if (companionClient?.readThreadMarkdown === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; thread markdown resource is unavailable.');
        }
    ...
    server.registerResource('sidetrack-thread-markdown', new ResourceTemplate('sidetrack://thread/{threadId}/markdown', { list: undefined }), {
    ...
    const dispatchRead = async (uri, variables) => {
        const dispatchId = requireVariable(variables, 'dispatchId');
    ...
    server.registerResource('sidetrack-dispatch', new ResourceTemplate('sidetrack://dispatch/{dispatchId}', { list: undefined }), {
    ...
    // a markdown resource.
    const workstreamContextRead = async (uri, variables) => {
        const workstreamId = requireVariable(variables, 'workstreamId');
        const snapshot = await reader.readSnapshot();
        ...
        const threads = snapshot.threads.filter((thread) => thread.primaryWorkstreamId === workstreamId);
    ...
    server.registerResource('sidetrack-workstream-context', new ResourceTemplate('sidetrack://workstream/{workstreamId}/context', { list: undefined }), {
