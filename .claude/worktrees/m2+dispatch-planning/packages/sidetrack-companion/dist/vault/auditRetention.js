...
const gzipFile = async (path, target) => {
    await pipeline(createReadStream(path), createGzip(), createWriteStream(target));
};
export const enforceRetention = async (vaultRoot, opts = {}, now = new Date()) => {
    ...
    for (const name of names.filter((candidate) => candidate.endsWith('.jsonl'))) {
        ...
        const rotated = join(auditRoot, `${basename(name, '.jsonl')}.${now.toISOString().replace(/[:.]/gu, '-')}.jsonl.gz`);
