...
import { parseMarkdownLockSentinel, renderPromotedThreadMarkdown, renderThreadMarkdown, renderWorkstreamMarkdown, } from './markdownProjection.js';
import { captureEventSchema, auditEventSchema, codingAttachTokenSchema, codingSessionSchema, dispatchEventRecordSchema, dispatchLinkSchema, reviewEventRecordSchema, settingsDocumentSchema, } from '../http/schemas.js';
...
const createDefaultSettings = (revision = '0') => settingsDocumentSchema.parse({
...
const writeMarkdownProjection = async (path, body) => {
    try {
        await mkdir(join(path, '..'), { recursive: true });
...
const isMissingPathError = (error) => typeof error === 'object' &&
    ...
    'code' in error &&
    error.code === 'ENOENT';
const readMarkdownLockSentinel = async (path) => {
    try {
...
const incrementSettingsRevision = (revision) => {
    if (!/^\d+$/u.test(revision)) {
        throw new Error('Settings revision must be numeric to increment.');
    }
    return (BigInt(revision) + 1n).toString();
};
const readSettingsDocument = async (path) => {
    try {
        const raw = await readFile(path, 'utf8');
        return settingsDocumentSchema.parse(JSON.parse(raw));
    }
    catch (error) {
        if (isMissingPathError(error)) {
...
const readDispatchFile = async (path) => {
    const raw = await readFile(path, 'utf8');
    return raw
        .split('\n')
...
const parseDispatchLinkLine = (line) => {
    const trimmed = line.trim();
    if (trimmed.length === 0) {
        return undefined;
    }
...
const readDispatchLinkFile = async (path) => {
    const raw = await readFile(path, 'utf8');
    return raw
        .split('\n')
...
const readAllDispatchLinks = async (bacRoot) => {
    ...
    return (await Promise.all(files.map((name) => readDispatchLinkFile(join(linkRoot, name))))).flat();
};
...
const readReviewFile = async (path) => {
    const raw = await readFile(path, 'utf8');
    return raw
        .split('\n')
        .map(parseReviewLine)
...
const readEventFile = async (path) => {
    const raw = await readFile(path, 'utf8');
    return raw
        .split('\n')
        .map(parseEventLine)
...
export const createVaultWriter = (vaultPath) => {
    ...
    const audit = async (event) => {
        await appendJsonLine(join(bacRoot, 'audit', `${dateStamp(new Date(event.timestamp))}.jsonl`), event);
    };
    ...
    const readWorkstreamTitle = async (workstreamId) => {
        try {
    ...
    return {
        ...
        async writeCaptureEvent(input, requestId) {
            ...
            await appendJsonLine(join(bacRoot, 'events', `${dateStamp(new Date(input.capturedAt))}.jsonl`), event);
        ...
        async writeDispatchEvent(input, requestId) {
            await ensureVaultPresent();
            const timestamp = new Date().toISOString();
            await appendJsonLine(join(bacRoot, 'dispatches', `${dateStamp(new Date(input.createdAt))}.jsonl`), input);
            await audit({
                requestId,
        ...
        async linkDispatchToThread(input, requestId) {
            ...
            await appendJsonLine(join(bacRoot, 'dispatch-links', `${dateStamp(new Date(linkedAt))}.jsonl`), record);
            await audit({
                requestId,
                route: 'linkDispatch',
                outcome: 'success',
        ...
        async readDispatchEvents(query) {
            ...
            const events = (await Promise.all(names
                ...
                .reverse()
                .slice(0, 100)
                .map((name) => readDispatchFile(join(dispatchRoot, name))))).flat();
            return events
        ...
        async readAuditEvents(query) {
            ...
            const events = (await Promise.all(names
                ...
                .reverse()
                .slice(0, 100)
                .map((name) => readAuditFile(join(auditRoot, name))))).flat();
            return events
        ...
        async writeReviewEvent(input, requestId) {
            await ensureVaultPresent();
            const timestamp = new Date().toISOString();
            await appendJsonLine(join(bacRoot, 'reviews', `${dateStamp(new Date(input.createdAt))}.jsonl`), input);
            await audit({
                requestId,
                route: 'recordReview',
                outcome: 'success',
        ...
        async readReviewEvents(query) {
            ...
            for (const name of names
                ...
                events.push(...fileEvents.filter((event) => (sinceMillis === undefined || Date.parse(event.createdAt) >= sinceMillis) &&
        ...
        async upsertThread(input, requestId) {
            ...
            const previousWorkstreamId = typeof existingThread?.['primaryWorkstreamId'] === 'string'
            ...
            const previousResearchMode = existingThread?.['lastResearchMode'] === 'deep-research' ||
            ...
            const promotedForFirstTime = existingThread !== undefined &&
            ...
            if (!(await readMarkdownLockSentinel(threadMarkdownPath))) {
                if (promotedForFirstTime && promotedWorkstreamId !== undefined) {
                    ...
                    await writeMarkdownProjection(threadMarkdownPath, renderPromotedThreadMarkdown(thread, turns, workstreamTitle, timestamp));
                }
                else if (existingThread === undefined || previousWorkstreamId === undefined) {
                    await writeMarkdownProjection(threadMarkdownPath, renderThreadMarkdown(thread));
                }
            }
        ...
        async createWorkstream(input, requestId) {
            ...
            await writeMarkdownProjection(join(bacRoot, 'workstreams', `${bac_id}.md`), renderWorkstreamMarkdown(workstream));
            if (input.parentId !== undefined) {
                ...
                await writeMarkdownProjection(join(bacRoot, 'workstreams', `${parentId}.md`), renderWorkstreamMarkdown(updatedParent));
            }
        ...
        async updateWorkstream(workstreamId, input, requestId) {
            ...
            const previousParentId = typeof existing['parentId'] === 'string' ? existing['parentId'] : undefined;
            const revision = createRevision();
            const timestamp = new Date().toISOString();
            ...
            await writeMarkdownProjection(join(bacRoot, 'workstreams', `${workstreamId}.md`), renderWorkstreamMarkdown(updated));
            const parentChanged = (wantsDetach && previousParentId !== undefined) ||
            ...
            if (parentChanged) {
                if (previousParentId !== undefined) {
                    ...
                    const updatedPrev = {
                        ...
                        children: readStringArray(previousParent['children']).filter((childId) => childId !== workstreamId),
                        revision: createRevision(),
                        updatedAt: timestamp,
                    };
                    await writeJson(previousParentPath, updatedPrev);
                    await writeMarkdownProjection(join(bacRoot, 'workstreams', `${previousParentId}.md`), renderWorkstreamMarkdown(updatedPrev));
                }
                if (wantsReparent) {
                    ...
                    await writeMarkdownProjection(join(bacRoot, 'workstreams', `${nextParentId}.md`), renderWorkstreamMarkdown(updatedNext));
                }
            }
            await audit({
                requestId,
        ...
        async deleteWorkstream(workstreamId, requestId) {
            ...
            if (previousParentId !== undefined) {
                ...
                try {
                    ...
                    await writeJson(parentPath, updatedParent);
                    await writeMarkdownProjection(join(bacRoot, 'workstreams', `${previousParentId}.md`), renderWorkstreamMarkdown(updatedParent));
                }
                catch (error) {
                    if (!isMissingPathError(error))
                        throw error;
        ...
        async registerCodingSession(input, requestId) {
            ...
            try {
                ...
                tokenRecord = codingAttachTokenSchema.parse(JSON.parse(raw));
            }
            catch (error) {
                if (isMissingPathError(error)) {
        ...
        async listCodingSessions(query) {
            ...
            for (const name of names.filter((candidate) => candidate.endsWith('.json'))) {
                try {
                    ...
                    const parsed = codingSessionSchema.safeParse(JSON.parse(raw));
                    if (parsed.success) {
            ...
            const filtered = query.workstreamId === undefined
                ? sessions
        ...
        async detachCodingSession(bac_id, requestId) {
            ...
            try {
                const raw = await readFile(path, 'utf8');
                existing = codingSessionSchema.parse(JSON.parse(raw));
            }
            catch (error) {
                if (isMissingPathError(error)) {
