...
import { defaultAllowedTools, isAllowed, readTrust, writeTrust, } from '../auth/workstreamTrust.js';
...
import { appendEntry as appendEntryRaw, gcEntries as gcEntriesRaw, readIndex, tombstoneByThread as tombstoneByThreadRaw, } from '../recall/indexFile.js';
import { buildLexicalIndex, rank, rankHybrid, } from '../recall/ranker.js';
...
import { buildSignals } from '../suggestions/buildSignals.js';
import { scoreSuggestions } from '../suggestions/score.js';
import { vectorFromEvents } from '../sync/causal.js';
...
const syncSummaryDeps = (replica, sync, syncMaterializerHealth) => replica === undefined
    ? {}
    : {
        syncSummary: () => {
            ...
            const materializersBlock = materializers === undefined || Object.keys(materializers).length === 0
                ? {}
                : { materializers };
...
// the wire.
const baseVectorForAggregate = async (eventLog, aggregateId) => vectorFromEvents(await eventLog.readByAggregate(aggregateId));
...
import { CodingAttachTokenInvalidError, CodingSessionNotFoundError, SettingsRevisionConflictError, WorkstreamHasChildrenError, createVaultWriter, } from '../vault/writer.js';
import { createProblem } from './problem.js';
import { annotationCreateSchema, annotationListQuerySchema, annotationUpdateSchema, auditListQuerySchema, captureEventSchema, codingAttachTokenCreateSchema, codingSessionListQuerySchema, codingSessionRegisterSchema, dispatchEventSchema, dispatchLinkRequestSchema, dispatchListQuerySchema, queueCreateSchema, reminderCreateSchema, reminderUpdateSchema, recallIndexSchema, recallGcSchema, recallQuerySchema, reviewDraftEventBatchSchema, reviewDraftListQuerySchema, reviewEventSchema, reviewListQuerySchema, settingsPatchSchema, suggestionQuerySchema, threadUpsertSchema, turnsQuerySchema, workstreamCreateSchema, workstreamUpdateSchema, autoUpdateSchema, bucketsPutSchema, workstreamTrustPutSchema, } from './schemas.js';
...
const mutationResponse = (result, requestId) => ({
    data: {
        ...result,
        requestId,
    },
...
const isLocalHost = (host) => Boolean(host && /^(127\.0\.0\.1|localhost|\[::1\])(?::\d+)?$/u.test(host));
const requireIdempotencyKey = (request) => {
    const key = request.headers['idempotency-key'];
    if (typeof key !== 'string' || key.length < 8) {
        throw new HttpRouteError(400, 'VALIDATION_ERROR', 'Validation failed.', 'Idempotency-Key header is required.');
    }
    return key;
};
const runIdempotent = async (context, route, key, operation, 
...
const getValidationIssues = (error) => {
...
const recallIndexPath = (vaultRoot) => join(vaultRoot, '_BAC', 'recall', 'index.bin');
const lexicalIndexCache = new Map();
const readWorkstreamThreadIds = async (vaultRoot, workstreamId) => {
...
const readWorkstreams = async (vaultRoot) => {
...
const readVaultMarkdown = async (vaultRoot, kind, bacId) => {
...
const writerForBucket = async (context, input) => {
...
const requireWorkstreamTrust = async (context, workstreamId, tool) => {
    if (workstreamId === undefined || context.vaultRoot === undefined) {
        return;
    }
    if (!isAllowed(workstreamId, tool, await readTrust(context.vaultRoot))) {
        throw new HttpRouteError(403, 'WORKSTREAM_NOT_TRUSTED', 'Workstream has not trusted this MCP write tool.', `${tool} is not allowed for workstream ${workstreamId}.`);
    }
};
const mcpToolHeader = (request) => {
...
const isSelectorCanary = (value) => value === 'ok' || value === 'warning' || value === 'failed';
const firstCaptureWarningMessage = (value) => {
...
const captureHealthSummary = async (vaultRoot) => {
        ...
        for (const line of raw.split('\n')) {
            try {
                ...
                if (typeof event.provider === 'string' && typeof event.capturedAt === 'string') {
                    ...
                    if (!Number.isNaN(capturedMillis) &&
                        capturedMillis >= since24h &&
                        selectorCanary !== null) {
                        if (selectorCanary === 'ok')
                            current.ok24h += 1;
                        if (selectorCanary === 'warning')
                            current.warn24h += 1;
                        if (selectorCanary === 'failed')
                            current.fail24h += 1;
                    }
    ...
    return {
        ...
        queueDepthHint: null,
        droppedHint: null,
        providers: [...providerRows.values()].sort((left, right) => (right.lastCaptureAt ?? '').localeCompare(left.lastCaptureAt ?? '')),
        recentWarnings: recentWarnings
            .sort((left, right) => right.capturedAt.localeCompare(left.capturedAt))
            .slice(0, 10),
    };
};
const readThreadWorkstreamId = async (vaultRoot, threadId) => {
    try {
        const parsed = JSON.parse(await readFile(join(vaultRoot, '_BAC', 'threads', `${threadId}.json`), 'utf8'));
...
const readThreadMetadata = async (vaultRoot, threadId) => {
    try {
...
const objectRecord = (value) => typeof value === 'object' && value !== null && !Array.isArray(value)
    ? value
    : undefined;
const parseThreadUpsertBody = async (vaultRoot, body) => {
    ...
    const existing = objectRecord(JSON.parse(await readFile(join(vaultRoot, '_BAC', 'threads', `${bacId}.json`), 'utf8')));
    ...
    return threadUpsertSchema.parse({
        ...
        ...(rawWorkstreamId === null
            ...
            : typeof rawWorkstreamId === 'string'
                ...
                : {}),
        lastSeenAt: typeof existing['lastSeenAt'] === 'string'
            ? existing['lastSeenAt']
...
const routes = [
    ...
    },
    {
        ...
        handle: async (_request, requestId, _match, context) => {
            ...
            const relayBlock = context.sync?.relay === undefined
                ? undefined
                : {
                    ...context.sync.relay,
                    ...(relayLive === null
    ...
    },
    {
        ...
        handle: async (_request, _requestId, _match, context) => [
            200,
            {
                data: await (context.serviceInstaller ?? pickInstaller()).install(buildServiceInstallOptions(context)),
            },
        ],
    },
    {
    ...
    },
    {
        ...
        handle: async (request, _requestId, _match, context) => {
            if (context.allowAutoUpdate !== true) {
                throw new HttpRouteError(403, 'AUTO_UPDATE_DISABLED', 'Auto-update is disabled.', 'Start the companion with --allow-auto-update before invoking this endpoint.');
            }
    ...
    },
    {
        ...
        authRequired: true,
        handle: (_request, _requestId, _match, context) => Promise.resolve([200, { data: context.hygieneStatus ?? {} }]),
    },
    {
        method: 'POST',
    ...
    },
    {
        ...
        handle: async (request, requestId, match, context) => {
            ...
            const record = await context.vaultWriter.linkDispatchToThread({ dispatchId: match.bacId, threadId: body.threadId }, requestId);
            if (context.eventLog !== undefined) {
                await context.eventLog
                    .appendServerObserved({
    ...
    },
    {
        ...
        handle: async (_request, _requestId, _match, context) => {
            if (context.eventLog === undefined) {
                throw new HttpRouteError(503, 'EVENT_LOG_UNAVAILABLE', 'Event log is not configured on this companion.');
            }
            const merged = await context.eventLog.readMerged();
            const dispatchEvents = merged.filter((event) => event.type === DISPATCH_RECORDED || event.type === DISPATCH_LINKED);
    ...
    },
    {
        ...
        handle: async (request, _requestId, match, context) => {
            ...
            const timeoutMs = Number.isFinite(requested) && requested > 0 ? Math.min(requested, 120_000) : 60_000;
            const vaultRoot = context.vaultRoot;
            const includeTurn = url.searchParams.get('includeLatestAssistantTurn') !== 'false';
            const buildResponse = async (link) => {
                if (link === null) {
                    return {
                        dispatchId,
                        matched: false,
                ...
                const meta = vaultRoot === undefined ? null : await readThreadMetadata(vaultRoot, link.threadId);
                ...
                const dispatchTargetProviders = ['chatgpt', 'claude', 'gemini'];
                const sanitizedProvider = dispatchTargetProviders.find((candidate) => candidate === meta?.provider);
                const thread = {
            ...
            const result = await new Promise((resolve) => {
                ...
                const unsubscribe = context.vaultChanges?.subscribe((event) => {
    ...
    },
    {
        ...
        handle: async (request, _requestId, match, context) => {
            ...
            return await runIdempotent(context, 'reviewDraftEvent', idempotencyKey, async () => {
                ...
                const threadUrl = input.threadUrl ?? (await readReviewDraft(vaultRoot, threadId))?.threadUrl ?? '';
        ...
        handle: async (request, _requestId, _match, context) => {
            ...
            return await runIdempotent(context, 'createAnnotation', idempotencyKey, async () => {
                ...
                if ('term' in input) {
                    ...
                    let threadUrl = input.url;
                    let pageTitle = input.pageTitle;
                    ...
                    const cleanedPolicy = policy === undefined
                        ? undefined
                        : {
    ...
    },
    {
        ...
        handle: async (_request, _requestId, _match, context) => {
            ...
            const merged = await context.eventLog.readMerged();
            const annotationEvents = merged.filter((event) => event.type === ANNOTATION_CREATED ||
    ...
    },
    {
        ...
        handle: async (request, _requestId, _match, context) => {
            ...
            const threadIds = query.workstreamId === undefined
                ? undefined
            ...
            const lexical = cached !== undefined &&
            ...
            const hybridRanked = rankHybrid(query.q, queryEmbedding ?? new Float32Array(384), index.items, new Date(), {
                limit: query.limit,
                lexical,
                ...(threadIds === undefined
                    ? {}
                    : { workstreamMembership: (threadId) => threadIds.has(threadId) }),
            });
            ...
            const ranked = hybridRanked.length > 0
                ? hybridRanked
            ...
            const enriched = await Promise.all(ranked.map(async (item) => {
                ...
                if (info === undefined) {
                    try {
                        const threadFile = await readFile(join(vaultRoot, '_BAC', 'threads', `${item.threadId}.json`), 'utf8');
                        const parsed = JSON.parse(threadFile);
                        info = {
                ...
                if (info.title.length > 0)
                    additions['title'] = info.title;
                if (info.threadUrl.length > 0)
                    additions['threadUrl'] = info.threadUrl;
    ...
    },
    {
        ...
        handle: async (request, _requestId, _match, context) => {
            ...
            const data = context.recallLifecycle !== undefined
    ...
    },
    {
        ...
        handle: async (request, requestId, _match, context) => {
            ...
            return await runIdempotent(context, 'appendEvent', idempotencyKey, async () => {
                ...
                const eventLogAppended = context.eventLog === undefined
                    ? false
                    : await context.eventLog
                        .appendServerObserved({
                        clientEventId: idempotencyKey,
    ...
    },
    {
        ...
        handle: async (_request, requestId, match, context) => {
            ...
            if (mcpToolHeader(_request) === 'sidetrack.threads.archive') {
                await requireWorkstreamTrust(context, await readThreadWorkstreamId(vaultRoot, match.bacId), 'sidetrack.threads.archive');
            }
            ...
            const tombstoneByThread = lifecycle === undefined
                ? (threadId) => tombstoneByThreadRaw(recallIndexPath(vaultRoot), threadId)
                : (threadId) => lifecycle.tombstoneByThread(threadId);
    ...
    },
    {
        ...
        handle: async (_request, requestId, match, context) => {
            ...
            if (mcpToolHeader(_request) === 'sidetrack.threads.unarchive') {
                await requireWorkstreamTrust(context, await readThreadWorkstreamId(requireVaultRoot(context), match.bacId), 'sidetrack.threads.unarchive');
            }
    ...
    },
    {
        ...
        handle: async (_request, _requestId, match, context) => {
            ...
            const record = (await readTrust(requireVaultRoot(context))).find((item) => item.workstreamId === match.workstreamId);
            return [
                200,
                {
                    data: {
                        ...
                        allowedTools: record === undefined ? [...defaultAllowedTools()] : [...record.allowedTools],
                    },
                },
            ];
        },
    ...
    },
    {
        ...
        handle: async (_request, requestId, match, context) => {
            ...
            return [
                200,
                mutationResponse(await context.vaultWriter.bumpWorkstream(match.bacId, requestId), requestId),
            ];
        },
    },
    {
        ...
        handle: async (request, requestId, match, context) => {
            ...
            const result = await context.vaultWriter.updateWorkstream(match.workstreamId, input, requestId);
            ...
            if (context.eventLog !== undefined) {
                const vaultRoot = requireVaultRoot(context);
                try {
                    const raw = await readFile(join(vaultRoot, '_BAC', 'workstreams', `${match.workstreamId}.json`), 'utf8');
        ...
        handle: async (_request, requestId, match, context) => {
            ...
            try {
                ...
                if (error instanceof WorkstreamHasChildrenError) {
                    throw new HttpRouteError(409, 'WORKSTREAM_HAS_CHILDREN', `Cannot delete — ${String(error.childCount)} child workstream(s) remain. Detach or delete children first.`);
                }
                throw error;
            }
        },
    ...
    },
    {
        ...
        handle: async (_request, requestId, match, context) => {
            ...
            const result = await context.vaultWriter.detachCodingSession(match.codingSessionId, requestId);
            return [200, { data: result }];
        },
    },
    ...
    // a ScopedResult-shaped envelope with `scope: 'companion-extended'`.
    {
        ...
        handle: async (request, requestId, _match, context) => {
            ...
            if (body === null || typeof body !== 'object' || !Array.isArray(body.events)) {
                throw new HttpRouteError(400, 'INVALID_REQUEST', 'Body must be { events: AcceptedEvent[] }.');
            }
            const imported = [];
            const skipped = [];
            for (const candidate of body.events) {
                ...
                const sanitizedEvent = sanitizedPayload === event.payload
                    ? event
    ...
    },
    {
        ...
        handle: async (request, requestId, _match, context) => {
            if (context.timelineStore === undefined) {
                throw new HttpRouteError(503, 'TIMELINE_NOT_WIRED', 'Timeline projection is not configured.');
            }
            ...
            const since = sinceRaw === undefined ? undefined : isDateOnly(sinceRaw) ? `${sinceRaw}T00:00:00.000Z` : sinceRaw;
            const until = untilRaw === undefined ? undefined : isDateOnly(untilRaw) ? `${untilRaw}T23:59:59.999Z` : untilRaw;
            ...
            const inRange = days.filter((d) => {
                if (since !== undefined && d < since.slice(0, 10))
                    return false;
                if (until !== undefined && d > until.slice(0, 10))
                    return false;
                return true;
            });
            const items = [];
            ...
            const overlapsRange = (entry) => {
                if (since !== undefined && entry.lastSeenAt < since)
                    return false;
                if (until !== undefined && entry.firstSeenAt > until)
                    return false;
                return true;
            };
    ...
    // the side panel + MCP can render partial-data states honestly.
    {
        ...
        handle: async (request, requestId, _match, context) => {
            ...
            if (workstreamId !== undefined) {
                ...
                const keepNodeIds = new Set([wsNodeId]);
                for (const n of nodes) {
                    if (n.metadata['workstreamId'] === workstreamId)
                        keepNodeIds.add(n.id);
                }
                // Pull in edge endpoints reachable from the kept set in one
                // hop so the projection is comprehensible.
                for (const e of edges) {
                    if (keepNodeIds.has(e.fromNodeId))
                        keepNodeIds.add(e.toNodeId);
                    if (keepNodeIds.has(e.toNodeId))
                        keepNodeIds.add(e.fromNodeId);
                }
...
export const createCompanionHttpServer = (context) => createServer((request, response) => {
    void handleRequest(request, response, context);
});
export const handleRequest = async (request, response, context) => {
    ...
    if (url === undefined || route === undefined) {
        sendJson(response, 404, createProblem({
            status: 404,
            code: 'NOT_FOUND',
            title: 'Not found',
    ...
    if (!isLocalHost(request.headers.host) || !isAllowedOrigin(request.headers.origin)) {
        sendJson(response, 403, createProblem({
            status: 403,
            code: 'LOOPBACK_ONLY',
    ...
    if (route.authRequired) {
        ...
        if (typeof actualKey !== 'string' ||
            ...
            sendJson(response, 401, createProblem({
                status: 401,
    ...
    try {
        ...
        const vaultUnavailable = error instanceof Error && error.message === 'Vault path is unavailable.';
        ...
        sendJson(response, status, createProblem({
            status,
...
export const startHttpServer = async (server, port) => new Promise((resolve, reject) => {
