...
// semver.
export const EXTRACTOR_MANIFEST = [
    {
        extractorId: 'legacy',
...
const capabilityScore = (extractorId) => {
...
// revision yet."
export const selectActiveRevision = (candidates) => {
    ...
    return live.reduce((best, candidate) => {
        ...
        if (candidate.extractionSchemaVersion > best.extractionSchemaVersion)
            return candidate;
        if (candidate.extractionSchemaVersion < best.extractionSchemaVersion)
            return best;
        ...
        if (cd === undefined && bd === undefined)
            return best;
        if (cd === undefined)
            return best;
        if (bd === undefined)
            return candidate;
        if (cd.replicaId !== bd.replicaId)
            return cd.replicaId < bd.replicaId ? candidate : best;
