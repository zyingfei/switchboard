...
export class RecallModelMissingError extends Error {
    ...
    constructor(message, offline, cacheDir) {
        super(message);
...
let resolvedDevice = 'unknown';
let resolvedAccelerator = 'unknown';
export const getResolvedEmbedderDevice = () => resolvedDevice;
export const getResolvedEmbedderAccelerator = () => resolvedAccelerator;
const log = (message) => {
...
export const getEmbedder = async () => {
    if (extractorPromise === undefined) {
        ...
        extractorPromise = (async () => {
            ...
            const dtypeCandidates = ['q8', 'fp16', 'fp32'];
            const errors = [];
            ...
            for (const dtype of dtypeCandidates) {
                try {
                    ...
                    log(`[recall] loaded embedding model ${MODEL_ID} (cpu/${resolvedAccelerator}/${dtype}/rev=${revision.slice(0, 7)}) in ${String(Math.round(performance.now() - started))}ms`);
                    return pipe;
                }
                catch (error) {
            ...
            const looksLikeModelMissing = offlineAtLoad ||
                /no such file|ENOENT|Could not locate|failed to fetch|Could not load model|404/i.test(joined);
            if (looksLikeModelMissing) {
                ...
                throw new RecallModelMissingError(`[recall] embedding model ${MODEL_ID} is not available (offline=${String(offlineAtLoad)}, cacheDir=${cacheDirAtLoad}). Tried: ${joined}`, offlineAtLoad, cacheDirAtLoad);
            }
...
const isTestEmbedderEnabled = () => typeof process !== 'undefined' && process.env?.['SIDETRACK_TEST_EMBEDDER'] === '1';
const testEmbed = (text) => {
