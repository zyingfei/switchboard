...
const readString = (buffer, cursor) => {
...
export const readIndex = async (path) => {
    ...
    try {
        ...
        cursor.offset += 4;
        const header = JSON.parse(buffer.subarray(cursor.offset, cursor.offset + headerLength).toString('utf8'));
        ...
        const capabilities = Array.isArray(header.schemaCapabilities)
            ? header.schemaCapabilities.filter((v) => typeof v === 'string')
            : SCHEMA_CAPABILITIES;
        return {
...
const sortEntriesCanonically = (items) => {
    ...
    return [...items].sort((left, right) => {
        ...
        if (byId !== 0)
            return byId;
        const byReplica = compareString(left.replicaId ?? DEFAULT_REPLICA, right.replicaId ?? DEFAULT_REPLICA);
        if (byReplica !== 0)
            return byReplica;
...
const encodeMetadata = (metadata) => encodeString(metadata === undefined ? '' : stableJsonStringify(metadata));
export const writeIndex = async (path, items, modelId, options = {}) => {
...
export const appendEntry = async (path, entry, modelId, options = {}) => {
...
export const upsertEntries = async (path, entries, modelId, options = {}) => {
    ...
    const keyOf = (entry) => `${entry.id} ${entry.replicaId ?? DEFAULT_REPLICA}`;
    ...
    await writeIndex(path, [...byKey.values()], modelId, revision === undefined ? {} : { modelRevision: revision });
...
export const replaceEntriesForSourceUnit = async (path, input, modelId, options = {}) => {
    const existing = await readIndex(path);
    ...
    const tagged = input.entries.map((entry) => ({
        ...entry,
...
export const gcEntries = async (path, validIds) => {
    const existing = await readIndex(path);
    if (existing === null) {
        return { removed: 0 };
    }
    const kept = existing.items.filter((item) => validIds.has(item.id));
    await writeIndex(path, kept, existing.modelId, existing.modelRevision === undefined ? {} : { modelRevision: existing.modelRevision });
...
export const tombstoneByThread = async (path, threadId) => {
    const existing = await readIndex(path);
    if (existing === null)
        return { tombstoned: 0 };
    let nextLamport = 1;
    ...
    let count = 0;
    const updated = existing.items.map((item) => {
        if (item.threadId !== threadId || item.tombstoned === true)
            return item;
        count += 1;
        const stamped = {
            ...item,
    ...
    await writeIndex(path, updated, existing.modelId, existing.modelRevision === undefined ? {} : { modelRevision: existing.modelRevision });
