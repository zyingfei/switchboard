...
const lockPath = (vaultRoot) => join(recallDir(vaultRoot), '.lock');
...
export class RecallLockHeldError extends Error {
    vaultRoot;
    pid;
    constructor(vaultRoot, pid) {
        super(`Another companion (pid ${String(pid)}) already owns the recall index for ${vaultRoot}. ` +
            'Stop the other process or wait for it to exit before starting a second companion.');
        this.vaultRoot = vaultRoot;
        this.pid = pid;
    }
}
export const acquireRecallProcessLock = async (vaultRoot) => {
...
// on every startup.
export const cleanupOrphanIndexTmpFiles = async (vaultRoot) => {
