...
const readCacheFile = async (state) => {
    let buffer;
    try {
...
const writeCacheFile = async (state, header, entries) => {
...
export const createEmbeddingCache = (vaultRoot, dim = 384) => {
    const state = {
    ...
    const get = async (key) => {
        ...
        if (cached === null)
            return null;
        if (cached.header.modelId !== key.modelId)
            return null;
        if (cached.header.modelRevision !== key.modelRevision)
            return null;
        return cached.entries.get(key.embedTextHash) ?? null;
    };
    const put = async (key, vector) => {
        ...
        else if (existing.header.modelId !== key.modelId ||
