...
export class AnchorBuilderError extends Error {
    reason;
    occurrenceCount;
    suggestedSelectionHints;
    constructor(reason, message, occurrenceCount = 0, suggestedSelectionHints) {
        super(message);
        this.reason = reason;
        this.occurrenceCount = occurrenceCount;
        this.suggestedSelectionHints = suggestedSelectionHints;
        this.name = 'AnchorBuilderError';
    }
}
const collapseWhitespace = (value) => value.replace(/\s+/g, ' ').trim().toLowerCase();
const findAllOccurrences = (haystack, needle) => {
    const indices = [];
    let from = 0;
...
const buildSuggestedHints = (turnText, occurrences) => {
    const out = [];
...
export const buildAnchorFromTerm = (input) => {
    const policy = {
        ...
        if (ordinal !== null) {
            ...
            if (normalisedHint.length > 0) {
                for (const offset of occurrences) {
                    const before = collapseWhitespace(input.turnText.slice(Math.max(0, offset - HINT_PRECEDING_WINDOW), offset));
    ...
    const suffix = input.turnText.slice(chosen + trimmedTerm.length, chosen + trimmedTerm.length + CONTEXT_CHARS);
    return {
        ok: true,
        anchor: {
            textQuote: {
