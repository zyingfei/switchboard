...
export const PROJECTED_EVENT_TYPES = Object.keys(PROJECTOR_REGISTRY);
export const runImportProjectors = async (deps, event) => {
...
const writeProjection = async (deps, params) => {
...
async function projectThreadAfterImport(deps, event) {
...
async function projectWorkstreamAfterImport(deps, event) {
    const bacId = event.aggregateId;
    if (typeof bacId !== 'string' || bacId.length === 0)
        return;
    const merged = await deps.eventLog.readByAggregate(bacId);
...
async function projectAnnotationAfterImport(deps, event) {
    const bacId = event.aggregateId;
    if (typeof bacId !== 'string' || bacId.length === 0)
        return;
    const merged = await deps.eventLog.readByAggregate(bacId);
...
async function projectQueueItemAfterImport(deps, event) {
    const bacId = event.aggregateId;
    if (typeof bacId !== 'string' || bacId.length === 0)
        return;
    const merged = await deps.eventLog.readByAggregate(bacId);
...
async function projectDispatchAfterImport(deps, event) {
    const bacId = event.aggregateId;
    if (typeof bacId !== 'string' || bacId.length === 0)
        return;
    const merged = await deps.eventLog.readByAggregate(bacId);
    ...
    if (recorded === undefined && link === undefined)
        return;
    await writeProjection(deps, {
        aggregate: 'dispatch',
        aggregateId: bacId,
...
async function projectReviewDraftAfterImport(deps, event) {
