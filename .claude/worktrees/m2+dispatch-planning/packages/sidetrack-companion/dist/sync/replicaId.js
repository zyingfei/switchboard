...
export const replicaIdPath = (vaultPath) => join(vaultPath, '_BAC', '.config', 'replica-id');
export const replicaSeqPath = (vaultPath) => join(vaultPath, '_BAC', '.config', 'replica-seq');
...
const legacyLamportPath = (vaultPath) => join(vaultPath, '_BAC', '.config', 'replica-lamport');
const UUID_V4 = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const ensureConfigDir = async (vaultPath) => {
...
const readReplicaIdFile = async (path) => {
    try {
...
export const loadOrCreateReplica = async (vaultPath) => {
    ...
    const nextSeq = () => enqueue(async () => {
        const next = highWaterMark + 1;
        await persist(next);
        highWaterMark = next;
        return next;
    });
    const peekSeq = () => highWaterMark;
    const observeSeq = (incoming) => enqueue(async () => {
...
//# sourceMappingURL=replicaId.js.map