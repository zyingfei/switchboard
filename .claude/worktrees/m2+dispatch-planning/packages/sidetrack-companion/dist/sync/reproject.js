...
const readVersionFile = async (vaultRoot) => {
    try {
...
const writeVersionFile = async (vaultRoot, version) => {
    const file = join(vaultRoot, ...VERSION_FILE_REL.split('/'));
    await mkdir(join(vaultRoot, '_BAC'), { recursive: true });
    const body = { version, writtenAt: new Date().toISOString() };
...
export const reprojectOnVersionMismatch = async (deps) => {
