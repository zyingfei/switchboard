...
import { CAPTURE_EXTRACTION_PRODUCED, isCaptureExtractionProducedPayload, } from '../../recall/extraction/events.js';
...
export const createExtractionMaterializer = (deps) => {
    ...
    const serializeBySource = async (sourceUnitId, work) => {
    ...
    const ingestRevision = async (revision) => {
        ...
        const dedupedHistory = history.some((h) => h.extractionRevisionId === revision.extractionRevisionId)
            ? history
        ...
        const status = indexed === winner.extractionRevisionId ? 'current' : 'stale';
        const next = {
    ...
    const handleRevision = async (revision) => {
        incInFlight();
        try {
    ...
    const handleEvent = async (event) => {
        ...
        else if (event.type === CAPTURE_EXTRACTION_PRODUCED &&
...
//# sourceMappingURL=extractionMaterializer.js.map