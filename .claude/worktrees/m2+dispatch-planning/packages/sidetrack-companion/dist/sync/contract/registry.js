...
// materializer.
export const KNOWN_MATERIALIZERS = new Set([
    'projection',
    'recall',
    'extraction',
    'timeline',
...
const projectionEntry = (eventType, surface) => ({
    eventType,
    surfaces: [
        {
            surface,
...
const reviewDraftEntries = REVIEW_DRAFT_EVENT_TYPES.map((eventType) => projectionEntry(eventType, 'review-draft-projection'));
...
export const REGISTERED_EVENT_TYPES = new Set(CONTRACT_REGISTRY.map((entry) => entry.eventType));
// All registry entries that route to the given materializer.
export const entriesForMaterializer = (name) => CONTRACT_REGISTRY.filter((entry) => entry.surfaces.some((surface) => surface.materializer === name));
// All event types that the given materializer should react to.
export const eventTypesForMaterializer = (name) => new Set(entriesForMaterializer(name).map((entry) => entry.eventType));
