...
import { decodeFrame, encodeFrame, PROTOCOL_VERSION, } from './relayProtocol.js';
...
export const startRelayServer = async (options = {}) => {
    ...
    const rendezvouses = new Map();
    const ensureRendezvous = (id) => {
    ...
    const replayBuffer = (state, target) => {
        trimBuffer(state);
    ...
    const handleSubscribe = (relay, frame) => {
    ...
    const handlePublish = (relay, frame) => {
        ...
        const state = ensureRendezvous(frame.rendezvous_id);
        const candidateBytes = frame.ciphertext.length + frame.nonce.length + frame.signature.length + 256;
