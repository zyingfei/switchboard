...
const RENDEZVOUS_SECRET_PATH_SEGMENTS = ['_BAC', '.config', 'sync-rendezvous.secret'];
const secretPath = (vaultRoot) => join(vaultRoot, ...RENDEZVOUS_SECRET_PATH_SEGMENTS);
const writeSecret = async (path, secret) => {
    await mkdir(dirname(path), { recursive: true });
    await writeFile(path, `${secret}\n`, { mode: 0o600 });
    await chmod(path, 0o600).catch(() => undefined);
};
export const ensureRendezvousSecret = async (vaultRoot, preferred) => {
...
//# sourceMappingURL=rendezvousSecret.js.map