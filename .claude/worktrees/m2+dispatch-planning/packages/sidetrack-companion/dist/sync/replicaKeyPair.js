...
import { generateReplicaKeyPair } from './relayCrypto.js';
...
const keyPairPath = (vaultPath) => join(vaultPath, '_BAC', '.config', 'replica-keypair.json');
const writeAtomic = async (path, body) => {
    const tmp = `${path}.${String(process.pid)}.${String(Date.now())}.tmp`;
    await writeFile(tmp, body, { encoding: 'utf8', mode: 0o600 });
    await rename(tmp, path);
};
export const loadOrCreateReplicaKeyPair = async (vaultPath) => {
...
//# sourceMappingURL=replicaKeyPair.js.map