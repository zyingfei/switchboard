...
const isSensitiveParam = (name) => {
    ...
    if (SENSITIVE_PARAM_NAMES.has(lower))
        return true;
...
const SEARCH_PATHS = new Set(['/', '/search']);
export const detectSearchUrl = (input) => {
    if (input.length === 0)
        return null;
    let parsed;
    try {
    ...
    if (!SEARCH_PATHS.has(normalized))
        return null;
...
export const sanitizeTimelineUrl = (input) => {
    if (input.length === 0)
        return input;
...
export const sanitizeTimelinePayload = (payload) => {
    const sanitizedUrl = sanitizeTimelineUrl(payload.url);
    const sanitizedCanonical = payload.canonicalUrl === undefined
        ? undefined
        : sanitizeTimelineUrl(payload.canonicalUrl);
    if (sanitizedUrl === payload.url &&
