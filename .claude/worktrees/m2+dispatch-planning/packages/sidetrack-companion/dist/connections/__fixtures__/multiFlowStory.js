...
// contiguous shingles).
// ---------------------------------------------------------------------------
const PYTHON_REPRO_BLOCK = 'import os\nfd = os.open("/tmp/probe", os.O_RDONLY)\ndata = os.copy_file_range(fd, fd2, 4096, 0)';
const SQL_MERGE_BLOCK = 'MERGE INTO accounts a USING new_balances n ON a.id = n.id WHEN MATCHED THEN UPDATE SET balance=n.balance';
const REDUCER_TS_BLOCK = 'export const buildConnectionsSnapshot = (input: ConnectionsInput): ConnectionsSnapshot => {';
// ---------------------------------------------------------------------------
// Event helpers
// ---------------------------------------------------------------------------
...
const toAcceptedEvents = (raw) => {
    const sorted = [...raw].sort((a, b) => {
        if (a.timeIso !== b.timeIso)
            return a.timeIso < b.timeIso ? -1 : 1;
...
// ---------------------------------------------------------------------------
export const buildMultiFlowFixture = () => ({
    events: buildEvents(),
    threads,
    workstreams,
    dispatches,
...
export const flowExclusiveNodes = (flow) => {
    ...
    if ('parentWorkstream' in f && f.parentWorkstream !== undefined)
        list.push(f.parentWorkstream);
    return list;
};
...
//# sourceMappingURL=multiFlowStory.js.map