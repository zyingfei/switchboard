...
const renderYamlFrontmatter = (fields) => {
    const lines = ['---'];
...
export const renderWorkstreamMarkdown = (input) => {
...
const ellipsizeTurnText = (value, maxChars = 2000) => {
    if (value.length <= maxChars) {
        return value;
    }
    return `${value.slice(0, maxChars).trimEnd()}\n\n[...]`;
};
const turnHeading = (role) => {
    if (role === 'user') {
        return 'User';
    }
    if (role === 'assistant') {
...
export const parseMarkdownLockSentinel = (content) => {
...
export const renderPromotedThreadMarkdown = (thread, turns, workstreamTitle, generatedAt = new Date().toISOString()) => {
    ...
    lines.push('', `_Generated by Sidetrack on ${generatedAt}; turns: ${String(turns.length)}. Edit this file to override the auto-projection — Sidetrack will not overwrite a hand-edited file (sentinel: bac_locked: true in frontmatter)._`, '');
    return lines.join('\n');
};
//# sourceMappingURL=markdownProjection.js.map