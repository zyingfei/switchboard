...
import { isAllowed, readTrust, writeTrust, } from '../auth/workstreamTrust.js';
...
import { CodingAttachTokenInvalidError, CodingSessionNotFoundError, SettingsRevisionConflictError, createVaultWriter, } from '../vault/writer.js';
import { createProblem } from './problem.js';
import { annotationCreateSchema, annotationListQuerySchema, annotationUpdateSchema, auditListQuerySchema, captureEventSchema, codingAttachTokenCreateSchema, codingSessionListQuerySchema, codingSessionRegisterSchema, dispatchEventSchema, dispatchListQuerySchema, queueCreateSchema, reminderCreateSchema, reminderUpdateSchema, recallIndexSchema, recallGcSchema, recallQuerySchema, reviewEventSchema, reviewListQuerySchema, settingsPatchSchema, suggestionQuerySchema, threadUpsertSchema, turnsQuerySchema, workstreamCreateSchema, workstreamUpdateSchema, autoUpdateSchema, bucketsPutSchema, workstreamTrustPutSchema, } from './schemas.js';
...
const mcpToolHeader = (request) => {
    ...
    return ['bac.move_item', 'bac.queue_item', 'bac.bump_workstream', 'bac.archive_thread', 'bac.unarchive_thread'].find((tool) => tool === value);
};
...
const recentCaptureByProvider = async (vaultRoot) => {
    ...
    return threadUpsertSchema.parse({
        ...
        ...(rawWorkstreamId === null
            ...
            : typeof rawWorkstreamId === 'string'
                ...
                : {}),
        lastSeenAt: typeof existing['lastSeenAt'] === 'string' ? existing['lastSeenAt'] : new Date().toISOString(),
        ...
        handle: async (request, _requestId, _match, context) => {
            ...
            for (let index = 0; index < input.items.length; index += 1) {
                ...
                if (item !== undefined && embedding !== undefined) {
                    await appendEntry(recallIndexPath(vaultRoot), {
                        id: item.id,
    ...
    },
    {
        ...
        handle: async (_request, requestId, match, context) => {
            ...
            if (mcpToolHeader(_request) === 'bac.archive_thread') {
                await requireWorkstreamTrust(context, await readThreadWorkstreamId(vaultRoot, match.bacId), 'bac.archive_thread');
            }
    ...
    },
    {
        ...
        handle: async (_request, requestId, match, context) => {
            ...
            if (mcpToolHeader(_request) === 'bac.unarchive_thread') {
                await requireWorkstreamTrust(context, await readThreadWorkstreamId(requireVaultRoot(context), match.bacId), 'bac.unarchive_thread');
            }
