...
const readThreads = async (vaultRoot) => {
    const root = join(vaultRoot, '_BAC', 'threads');
    const names = await readdir(root).catch(() => []);
    const threads = await Promise.all(names
        .filter((name) => name.endsWith('.json'))
        .map((name) => readJson(join(root, name))));
    return threads.filter((thread) => thread !== null);
};
export const buildSignals = async (vaultRoot, threadId, workstreams, indexReader = readIndex, 
...
embedder = embed) => {
    ...
    const coldStartIds = [];
    const coldStartTexts = [];
    for (const workstream of workstreams) {
        const memberCount = threads.filter((candidate) => candidate.primaryWorkstreamId === workstream.id).length;
        if (memberCount === 0) {
            coldStartIds.push(workstream.id);
            coldStartTexts.push(`${workstream.title} ${workstream.description ?? ''}`.trim());
        }
    }
    const coldStartCentroids = new Map();
    ...
    for (const workstream of workstreams) {
        ...
        lexical[workstream.id] = Math.max(jaccard(threadTokens, wsTokens), containment(wsTokens, threadTokens));
        const memberIds = new Set(threads
            .filter((candidate) => candidate.primaryWorkstreamId === workstream.id)
            .map((candidate) => candidate.bac_id)
            .filter((id) => typeof id === 'string'));
        const memberCentroid = meanNormalized(index?.items
...
//# sourceMappingURL=buildSignals.js.map