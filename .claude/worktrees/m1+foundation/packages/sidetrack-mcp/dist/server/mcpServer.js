...
const toolText = (value) => `${JSON.stringify(value, null, 2)}\n`;
const asStructuredContent = (value) => ({
    content: [{ type: 'text', text: toolText(value) }],
    structuredContent: value,
});
const buildContextPack = (snapshot, workstreamId, includeQueueItems) => {
    const workstreams = workstreamId === undefined
        ? snapshot.workstreams
        : snapshot.workstreams.filter((workstream) => workstream.bac_id === workstreamId);
    const threads = snapshot.threads.filter((thread) => workstreamId === undefined || thread.primaryWorkstreamId === workstreamId);
    const queueItems = includeQueueItems
        ? snapshot.queueItems.filter((item) => workstreamId === undefined || item.targetId === workstreamId)
        : [];
    const threadLines = threads.map((thread) => `- ${thread.title ?? thread.threadUrl ?? thread.bac_id}`);
    const queueLines = queueItems.map((item) => `- ${item.text ?? item.bac_id}`);
    const checklistLines = workstreams.flatMap((workstream) => (workstream.checklist ?? []).map((item, index) => {
        const record = typeof item === 'object' && item !== null
            ? item
            : {};
        const marker = record.checked === true ? 'x' : ' ';
        const text = typeof record.text === 'string' ? record.text : `Checklist item ${String(index + 1)}`;
        return `- [${marker}] ${text}`;
    }));
    return [
...
export const createSidetrackMcpServer = (reader, companionClient) => {
    const server = new McpServer({
        name: 'sidetrack-mcp',
        version: '0.0.0',
    });
    server.registerTool('bac.recent_threads', {
    ...
    server.registerTool('bac.workstream', {
        description: 'Return a workstream subtree plus tracked items, queued asks, and checklist data.',
        inputSchema: { id: z.string().optional() },
    }, async ({ id }) => {
        const snapshot = await reader.readSnapshot();
        const workstreams = id === undefined
            ? snapshot.workstreams
            : snapshot.workstreams.filter((workstream) => workstream.bac_id === id);
        return asStructuredContent({
            workstreams,
            threads: snapshot.threads.filter((thread) => id === undefined || thread.primaryWorkstreamId === id),
            queuedItems: snapshot.queueItems.filter((item) => id === undefined || item.targetId === id),
            generatedAt: snapshot.generatedAt,
        });
    });
    server.registerTool('bac.context_pack', {
    ...
    server.registerTool('bac.search', {
    ...
    server.registerTool('bac.queued_items', {
        description: 'Return queued follow-ups from the live vault.',
        inputSchema: { scope: z.string().optional() },
    }, async ({ scope }) => {
        const snapshot = await reader.readSnapshot();
        return asStructuredContent({
            queuedItems: scope === undefined
                ? snapshot.queueItems
                : snapshot.queueItems.filter((item) => item.scope === scope),
            generatedAt: snapshot.generatedAt,
        });
    });
    server.registerTool('bac.inbound_reminders', {
        description: 'Return inbound reminders from the live vault.',
        inputSchema: { since: z.string().optional() },
    }, async ({ since }) => {
        const snapshot = await reader.readSnapshot();
        return asStructuredContent({
            reminders: since === undefined
                ? snapshot.reminders
                : snapshot.reminders.filter((reminder) => (reminder.detectedAt ?? '') >= since),
            generatedAt: snapshot.generatedAt,
        });
    });
    server.registerTool('bac.coding_sessions', {
        description: 'Return coding sessions registered by coding agents (Claude Code / Codex / Cursor). Filterable by workstreamId and status.',
        inputSchema: {
            workstreamId: z.string().optional(),
    ...
    server.registerTool('bac.coding_session_register', {
        description: "Register the current coding agent's session against a Sidetrack workstream. Call this once at the start of a coding session. The user provides the attach token; auto-detect cwd, branch, sessionId, and a short display name from your runtime — do not ask the user for those.",
        inputSchema: {
            token: z
                .string()
                .min(8)
        ...
        if (companionClient === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.coding_session_register is unavailable.');
        }
    ...
    server.registerTool('bac.move_item', {
        description: 'Move a tracked thread into a workstream. Pass workstreamId="" (empty string) to clear the assignment and park the thread back at the top level. Returns the updated thread bac_id + revision.',
        inputSchema: {
        ...
        if (companionClient?.moveThread === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.move_item is unavailable.');
        }
        const target = workstreamId === undefined || workstreamId.length === 0 ? undefined : workstreamId;
    ...
    server.registerTool('bac.queue_item', {
        description: "Park a follow-up question for the user. Scope determines where it lands: 'thread' (set targetId to a thread bac_id), 'workstream' (set targetId to a workstream bac_id), or 'global' (no target). Returns the queue item bac_id.",
        inputSchema: {
            ...
            targetId: z
                .string()
                .optional()
                .describe("Thread or workstream bac_id; required when scope != 'global', ignored otherwise."),
        },
    }, async ({ text, scope, targetId }) => {
        if (companionClient?.createQueueItem === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.queue_item is unavailable.');
        }
        if (scope !== 'global' && (targetId === undefined || targetId.length === 0)) {
            throw new Error(`bac.queue_item requires targetId when scope='${scope}'. Pass the thread or workstream bac_id, or use scope='global'.`);
        }
    ...
    server.registerTool('bac.bump_workstream', {
        ...
        if (companionClient?.bumpWorkstream === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.bump_workstream is unavailable.');
        }
        return asStructuredContent(await companionClient.bumpWorkstream({ bac_id }));
    });
    server.registerTool('bac.archive_thread', {
        description: 'Soft-archive a tracked thread. Idempotent.',
        inputSchema: { bac_id: z.string().min(1) },
    }, async ({ bac_id }) => {
        if (companionClient?.archiveThread === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.archive_thread is unavailable.');
        }
        return asStructuredContent(await companionClient.archiveThread({ bac_id }));
    });
    server.registerTool('bac.unarchive_thread', {
        description: 'Clear a thread soft-archive marker. Idempotent.',
        inputSchema: { bac_id: z.string().min(1) },
    }, async ({ bac_id }) => {
        if (companionClient?.unarchiveThread === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.unarchive_thread is unavailable.');
        }
        return asStructuredContent(await companionClient.unarchiveThread({ bac_id }));
    });
    server.registerTool('bac.list_dispatches', {
        ...
        if (companionClient?.listDispatches === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.list_dispatches is unavailable.');
        }
    ...
    server.registerTool('bac.list_audit_events', {
        ...
        if (companionClient?.listAuditEvents === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.list_audit_events is unavailable.');
        }
    ...
    server.registerTool('bac.list_workstream_notes', {
        ...
        if (companionClient?.listWorkstreamNotes === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.list_workstream_notes is unavailable.');
        }
        const items = await companionClient.listWorkstreamNotes({ workstreamId });
        return asStructuredContent({ items: [...items] });
    });
    server.registerTool('bac.list_annotations', {
        ...
        if (companionClient?.listAnnotations === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.list_annotations is unavailable.');
        }
    ...
    server.registerTool('bac.update_annotation', {
        ...
        if (companionClient?.updateAnnotation === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.update_annotation is unavailable.');
        }
        return asStructuredContent(await companionClient.updateAnnotation({ bac_id, note }));
    });
    server.registerTool('bac.delete_annotation', {
        description: 'Soft-delete an annotation. Idempotent; history is preserved.',
        inputSchema: { bac_id: z.string().min(1) },
    }, async ({ bac_id }) => {
        if (companionClient?.deleteAnnotation === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.delete_annotation is unavailable.');
        }
        return asStructuredContent(await companionClient.deleteAnnotation({ bac_id }));
    });
    server.registerTool('bac.read_thread_md', {
        ...
        if (companionClient?.readThreadMarkdown === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.read_thread_md is unavailable.');
        }
        return asStructuredContent(await companionClient.readThreadMarkdown({ bac_id }));
    });
    server.registerTool('bac.read_workstream_md', {
        ...
        if (companionClient?.readWorkstreamMarkdown === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.read_workstream_md is unavailable.');
        }
        return asStructuredContent(await companionClient.readWorkstreamMarkdown({ bac_id }));
    });
    server.registerTool('bac.recall', {
        ...
        if (companionClient?.recall === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.recall is unavailable.');
        }
    ...
    server.registerTool('bac.suggest_workstream', {
        ...
        if (companionClient?.suggestWorkstream === undefined) {
            throw new Error('sidetrack-mcp was started without --companion-url / --bridge-key; bac.suggest_workstream is unavailable.');
        }
    ...
    server.registerTool('bac.dispatches', {
    ...
    server.registerTool('bac.reviews', {
    ...
    server.registerTool('bac.turns', {
        description: 'Return the most-recent captured assistant/user turns for a thread, by threadUrl, deduped by ordinal.',
        inputSchema: {
            threadUrl: z.url(),
            limit: z.number().int().positive().max(50).optional(),
