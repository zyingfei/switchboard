import { isElementVisible, isPrivateFormElement, normalizeInlineWhitespace, visibleTextFromElement } from './domUtils';
...
const serializeList = (element: Element, depth = 0): string => {
  ...
  const bulletPrefix = element.tagName.toLowerCase() === 'ol';

  return items
    .map((item, index) => {
      const prefix = bulletPrefix ? `${index + 1}. ` : '- ';
      const indent = '  '.repeat(depth);
      const segments: string[] = [];
      const nestedBlocks: string[] = [];

      Array.from(item.childNodes).forEach((child) => {
        if (child instanceof Element && blockTags.has(child.tagName.toLowerCase()) && child.tagName.toLowerCase() !== 'code') {
          const serialized = child.tagName.toLowerCase() === 'ul' || child.tagName.toLowerCase() === 'ol'
...
const serializeDefaultBlock = (element: Element): string => {
  ...
  Array.from(element.childNodes).forEach((child) => {
    if (child instanceof Element && isElementVisible(child) && blockTags.has(child.tagName.toLowerCase())) {
      flushInline();
