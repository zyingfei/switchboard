...
const findChromeForTesting = async (): Promise<string | null> => {
  ...
  const executableNames = new Set(['Google Chrome for Testing', 'chrome', 'chrome.exe']);
...
export const launchExtensionRuntime = async (): Promise<ExtensionRuntime> => {
  ...
  const userDataDir = await mkdtemp(path.join(os.tmpdir(), 'bac-provider-capture-e2e-'));
  const useHeadless = process.env.BAC_E2E_HEADLESS !== '0';
