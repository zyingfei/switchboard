...
const data = await probe.evaluate(
  async ({ target }) => {
    ...
    if (localThread) {
      const r = await fetch(
        `http://127.0.0.1:${port}/v1/suggestions/thread/${localThread.bac_id}?limit=10`,
    ...
    const recallR = await fetch(
      `http://127.0.0.1:${port}/v1/recall/query?q=${encodeURIComponent('hacker news summary technology')}&limit=10`,
      { headers: { 'x-bac-bridge-key': key } },
    );
    const recall = (await recallR.json()).data;

    // 3. Find any "hackernews"/"hn"/"hacker news" workstream
    const hnWs = workstreams.filter(
      (w) => /hacker|hn/i.test(w.title || '') || /hacker|hn/i.test((w.tags || []).join(' ')),
    );
    ...
    return {
      ...
      workstreams: workstreams.map((w) => ({
        ...
        title: w.title,
        tags: w.tags,
        memberHint: threads.filter((t) => t.primaryWorkstreamId === w.bac_id).length,
      })),
      hnWs,
      suggestions,
      topRecall: Array.isArray(recall)
        ? recall.slice(0, 8).map((r) => ({
