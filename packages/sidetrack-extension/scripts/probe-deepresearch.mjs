...
const target =
  'https://chatgpt.com/g/g-p-69e9ab0f1e04819191397ca941f85cf1/c/69ea4331-7c44-832a-b8ad-90a454c47273';
const page =
  ctx.pages().find((p) => p.url() === target) ??
  ctx.pages().find((p) => p.url().startsWith('https://chatgpt.com/g/'));
if (!page) {
  console.log('no chatgpt page');
  process.exit(1);
}
      ...
      .map((el) => ({
        ...
        attrs: Object.fromEntries(
          Array.from(el.attributes || [])
            .filter((a) => /role|data-|aria-label|aria-describedby/.test(a.name))
            .map((a) => [a.name, a.value.slice(0, 60)]),
        ),
        text: (el.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 100),
      }));
  return {
    deepResearchPills: sample(
      document.querySelectorAll(
        '[data-testid*="research"], [aria-label*="research"], [class*="research"], [class*="report"]',
      ),
      6,
    ),
    citations: sample(
      document.querySelectorAll('[data-testid*="citation"], [class*="citation"], a[href*="ref="]'),
      6,
    ),
    ...
    modelButton: document
      .querySelector(
        '[aria-label="Switch model"], button[aria-haspopup="menu"][data-state="closed"]',
      )
      ?.textContent?.trim(),
...
console.log(
  ...
  data.longResponses.length,
  '— sample:',
  data.longResponses.slice(0, 3),
);
