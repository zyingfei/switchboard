...
const swCode = await sw.evaluate(async () => {
  ...
  return {
    hasListAnnotations:
      text.includes('listAnnotationsByUrl') || text.includes('annotation.listByUrl'),
...
const tabs = await sw.evaluate(() =>
  chrome.tabs
    .query({})
    .then((t) =>
      t.filter((x) => x.url?.includes('69fa8f0f')).map((x) => ({ id: x.id, url: x.url })),
    ),
);
    ...
    func: async () => {
      ...
      try {
        const settings = await new Promise((r) =>
          chrome.storage.local.get('sidetrack.settings', (v) => r(v['sidetrack.settings'])),
        );
        const port = settings?.companion?.port;
        const bridgeKey = settings?.companion?.bridgeKey;
        const resp = await fetch(
          `http://127.0.0.1:${port}/v1/annotations?url=${encodeURIComponent(location.href)}`,
