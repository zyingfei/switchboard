...
const page = b
  .contexts()[0]
  .pages()
  .find((p) => /chatgpt\.com\/c\//.test(p.url()));
const data = await page.evaluate(() => {
  ...
  for (const m of msgs) {
    ...
    candidates.push({
      ...
      attrs: Object.fromEntries(
        Array.from(m.attributes)
          .filter((a) => /model|slug/.test(a.name))
          .map((a) => [a.name, a.value]),
      ),
    });
  }
  // Look for any element with model text
  const allTexts = Array.from(document.querySelectorAll('span, button, div'))
    .filter((el) => {
  ...
  const meta = Array.from(document.querySelectorAll('meta[name], meta[property]'))
    .filter((m) => /model|chat/.test(m.getAttribute('name') || m.getAttribute('property') || ''))
    .map((m) => ({
