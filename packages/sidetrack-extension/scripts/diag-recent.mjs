...
const target =
  'https://chatgpt.com/g/g-p-69e90d50b60c8191ae921376538a5a30-ddia/c/69eacc53-a0d8-832f-bd39-20158bd86a01';
const data = await probeCtx.evaluate(
  async ({ target }) => {
    ...
    return {
      target_thread,
      targetReminders: reminders.filter(
        (r) => r.threadId === target_thread?.bac_id || r.threadUrl === target,
      ),
      targetRecentDispatches: recentDispatches.filter(
        (d) => d.threadId === target_thread?.bac_id || d.targetUrl === target,
      ),
      targetDispatchLinks: Object.fromEntries(
        Object.entries(dispatchLinks).filter(
          ([k, v]) => v?.threadId === target_thread?.bac_id || v?.threadUrl === target,
        ),
      ),
      storageKeys: Object.keys(all).filter(
        (k) => k.includes('reminder') || k.includes('dispatch') || k.includes('recent'),
      ),
    };
  },
  { target },
