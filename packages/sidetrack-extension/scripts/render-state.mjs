...
const bgState = await sp.evaluate(async () => {
  ...
  const t = r?.state?.threads?.find(
    (x) => x.threadUrl === 'https://gemini.google.com/app/45e8a50792869814',
  );
  return t
    ? { bac_id: t.bac_id, title: t.title, primaryWorkstreamId: t.primaryWorkstreamId }
    : null;
});
