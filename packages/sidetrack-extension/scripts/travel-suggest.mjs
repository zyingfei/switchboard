...
const data = await probeCtx.evaluate(
  async ({ target }) => {
    ...
    if (!thread)
      return { error: 'no local thread', sample: threads.slice(0, 3).map((t) => t.threadUrl) };
    ...
    const r = await fetch(
      `http://127.0.0.1:${port}/v1/suggestions/thread/${thread.bac_id}?limit=20&threshold=0`,
      {
    ...
    const r2 = await fetch(
      `http://127.0.0.1:${port}/v1/recall/query?q=${encodeURIComponent(thread.title || 'travel')}&limit=10`,
      {
        headers: { 'x-bac-bridge-key': key },
      },
    );
    ...
    return {
      ...
      allWorkstreams: workstreams.map((w) => ({
        bac_id: w.bac_id,
        title: w.title,
        members: threads.filter((t) => t.primaryWorkstreamId === w.bac_id).length,
      })),
      suggestions: sug.data,
      recallTopN: Array.isArray(recall)
        ? recall.slice(0, 6).map((r) => ({
...
for (const s of data.suggestions || []) {
  const wsName =
    (data.allWorkstreams || []).find((w) => w.bac_id === s.workstreamId)?.title || s.workstreamId;
  console.log(
    ' ',
    wsName.padEnd(30),
    'score=',
    s.score.toFixed(3),
    'lex=',
    s.breakdown.lexical.toFixed(3),
    'vec=',
    s.breakdown.vector.toFixed(3),
    'link=',
    s.breakdown.link,
  );
}
