...
const data = await probeCtx.evaluate(async () => {
  ...
  const target = threads.find(
    (t) => t.threadUrl === 'https://chatgpt.com/c/69f944f4-2150-832b-9da8-292d9963bc39',
  );
  let suggResp = null;
  if (target) {
    const r = await fetch(
      `http://127.0.0.1:${port}/v1/suggestions/thread/${target.bac_id}?limit=10&threshold=0`,
      { headers: { 'x-bac-bridge-key': key } },
    );
  ...
  return {
    ...
    targetThread: target
      ? {
          bac_id: target.bac_id,
          title: target.title,
          primaryWorkstreamId: target.primaryWorkstreamId,
        }
      : null,
    health,
    suggResp,
...
for (const w of data.workstreams || [])
  console.log(' ', w.bac_id, '|', w.members, 'members |', w.title);
console.log('\nSAMPLE THREADS:');
for (const t of data.sampleThreads || [])
  console.log(
    ' ',
    t.bac_id,
    '|',
    (t.primaryWorkstreamId || '(unassigned)').padEnd(20),
    '|',
    t.title,
  );
