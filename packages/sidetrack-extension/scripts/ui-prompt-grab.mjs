...
if (!sp) {
  console.error('side panel must be open');
  process.exit(1);
}
...
const sendToOpen = await sp.evaluate((title) => {
  const row = Array.from(document.querySelectorAll('.thread')).find((r) =>
  ...
  const btn =
    row.querySelector(
      '[aria-label^="Send to"], [aria-label="Send"], [class*="send-to-trigger"], [data-testid*="send-to"]',
    ) ??
...
const targetClicked = await sp.evaluate(() => {
  ...
  const codex =
    all.find((el) => /^codex$/i.test((el.textContent || '').trim()) && el.children.length <= 3) ??
    all.find(
      (el) =>
        /\bcodex\b/i.test((el.textContent || '').trim()) && (el.textContent || '').length < 20,
    );
  if (!codex) {
    return {
      ok: false,
...
const modalState = await sp.evaluate(() => {
  const dialog = document.querySelector(
    '[role="dialog"], .modal, .Modal, .composer, [aria-modal="true"]',
  );
  const tryPre =
...
console.log(
  'modal:',
  JSON.stringify({
    tag: modalState.dialogTag,
    cls: modalState.dialogCls,
    bodyLen: modalState.bodyLen,
  }).slice(0, 200),
);
