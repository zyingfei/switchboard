...
export const visibleTextFromElement = (element: Element): string => {
  ...
  const walker = doc.createTreeWalker(element, nodeFilter?.SHOW_TEXT ?? 4, {
    acceptNode(node) {
...
export const filterNestedMatches = (elements: readonly Element[]): Element[] => {
...
export const elementsForSelector = (
  doc: Document,
  selector: string,
  opts: { readonly filterNestedMatches?: boolean } = {},
): Element[] => {
...
export const sortElementsInDocumentOrder = <TValue extends { readonly element: Element }>(
