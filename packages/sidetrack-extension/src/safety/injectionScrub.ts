...
const PATTERNS: readonly InjectionPattern[] = [
  { id: 'ignore-previous', pattern: /\bignore\s+(?:the\s+)?(?:previous|prior|all|above)/iu },
  {
    ...
    pattern: /\bdisregard\s+(?:the\s+)?(?:previous|prior|all|above|system)/iu,
  },
  {
    id: 'forget-instructions',
    pattern: /\bforget\s+(?:everything|your\s+(?:instructions|training|prompt))/iu,
  },
  ...
  {
    id: 'stop-following',
    pattern: /\bstop\s+following\s+(?:your|the)\s+(?:rules|guidelines|instructions)/iu,
  },
