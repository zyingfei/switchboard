...
export const routeEnrichmentEngine = (
  ...
  // from "you loaded nothing AND the built-in model can't help here because of
  // the language" — the second needs the language named or the user reasonably
  // asks why a 'ready' Nano isn't being used.
  if (availability.nanoReady && !nanoCanServe(language)) {
