...
import {
  ...
  type ExtractionRevisionContent,
  sourceUnitIdFor,
} from './types.js';
...
// thread.
export const wrapCaptureAsLegacyRevisions = (
  event: AcceptedEvent,
): readonly ExtractionRevision[] => {
  if (event.type !== CAPTURE_RECORDED) return [];
  if (!isCaptureRecordedPayload(event.payload)) return [];
  const payload = event.payload;
  const out: ExtractionRevision[] = [];
  for (let index = 0; index < payload.turns.length; index += 1) {
    ...
    const content: ExtractionRevisionContent = {
      turns: [
        {
          ordinal,
          role,
