...
it('one valid manifest → status=loaded, audit collector:manifest-loaded fired', async () => {
  ...
  const col0 = collectors[0];
  if (col0 === undefined) throw new Error('expected a collector entry');
  expect(col0.status).toBe('loaded');
  expect(col0.manifest.id).toBe('sidetrack.test');
  expect(col0.rejectedReason).toBeUndefined();
...
it('manifest with requires-companion >=999.0.0 → load-failed + correct rejection + audit', async () => {
  ...
  const col0 = collectors[0];
  if (col0 === undefined) throw new Error('expected a collector entry');
  expect(col0.status).toBe('load-failed');
  expect(col0.rejectedReason).toBe('requires-companion-not-satisfied');
...
it('mid-run manifest change → re-evaluation fires collector:manifest-reloaded', async () => {
  ...
  const reloaded0 = reloaded[0];
  if (reloaded0 === undefined) throw new Error('expected a reloaded audit entry');
  expect(reloaded0.subject).toBe('sidetrack.test');
