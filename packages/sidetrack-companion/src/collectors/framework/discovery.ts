...
export const startDiscovery = async (opts: DiscoveryOpts): Promise<DiscoveryHandle> => {
  ...
  const handleChange = (filename: string): void => {
    ...
    if (parts.length !== 2 || namePart !== TOML_FILENAME || idPart === undefined) return;
    const id: string = idPart;
    if (!validCollectorId(id)) return;
  ...
  try {
    watcher = watch(collectorsRoot, { recursive: true }, (_event, filename) => {
      ...
      const rel = relative(collectorsRoot, join(collectorsRoot, filename as string));
      handleChange(rel);
    });
  } catch {
