...
describe('HTTP route table characterization (stage S1 pin)', () => {
  it('pins the exact ordered "METHOD pattern.source" list — order is dispatch behavior', () => {
    ...
    expect(actual).toEqual([
      ...
      'GET ^\\/v1\\/visits\\/inbox$',
      'GET ^\\/v1\\/visits\\/(?<canonicalUrl>[^/]+)\\/memberships$',
      ...
      'POST ^\\/v1\\/visits\\/(?<canonicalUrl>[^/]+)\\/memberships$',
      'POST ^\\/v1\\/visits\\/(?<canonicalUrl>[^/]+)\\/memberships\\/(?<workstreamId>[^/]+)\\/remove$',
      'POST ^\\/v1\\/visits\\/(?<canonicalUrl>[^/]+)\\/suggestions\\/decline$',
