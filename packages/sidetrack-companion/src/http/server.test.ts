...
describe('companion HTTP server', () => {
  ...
  it('GET /v1/collectors returns last_promoted_at: null when framework has not promoted yet', async () => {
    ...
    expect(result.status).toBe(200);
    const body = result.body as { collectors: readonly Record<string, unknown>[] };
  ...
  // The /v1/content/query route and its three HTTP contract tests
  // were removed in the Scope A /v2/recall migration (PR #217 F1
  // commit). Coverage moved to pipeline-intent.test.ts (intent +
  // source-profile resolution against the in-memory store) and the
  // 12 eval/recall-quality fixtures in src/recall-v2/eval/.
});
